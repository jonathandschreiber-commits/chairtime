...
export async function DELETE(request, context) {
  ...
  try {
    const backendResponse = await fetch(
      `${API_URL}/api/admin/blocked-time-series/${encodeURIComponent(
        seriesId
      )}`,
      {
        method: "DELETE",
      ...
      return NextResponse.json(
        {
          ...
          error:
            backendData?.detail ||
            "Blocked-time series could not be deleted.",
        },
        {
    ...
    return NextResponse.json({
      success: true,
      occurrences_deleted:
        backendData?.occurrences_deleted || 0,
    });
  } catch (error) {
    console.error(
      "Blocked-time series DELETE route failed:",
      error
    );
